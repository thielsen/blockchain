import os
import pytest

from sample.blockchain import *
from sample.crypto_utilities import *
from sample.block import *
from sample.transaction import *
from sample.node import *


BOB_PRIVATE = ''
BOB_PUBLIC = ''

ALICE_PRIVATE = ''
ALICE_PUBLIC = ''

TEST_WALLET_PRIVATE = ''
TEST_WALLET_PUBLIC = ''

def test_public_private_verification():
    alice_wallet = Wallet('./tests/alice_wallet.txt')
    bob_wallet = Wallet('./tests/bob_wallet.txt')
    signature = alice_wallet.sign_transaction(ALICE_PUBLIC, BOB_PUBLIC, 100)
    transaction = Transaction(ALICE_PUBLIC, BOB_PUBLIC, signature, 100)
    assert alice_wallet.verify_transaction(transaction)
    assert bob_wallet.verify_transaction(transaction)

def test_cannot_send_if_transactions_in_queue_are_too_much():
    alice_wallet = Wallet('./tests/alice_wallet.txt')
    alice_blockchain = BlockChain(ALICE_PUBLIC, './tests/blockchain.bin')
    alice_node = Node(alice_wallet, alice_blockchain)
    alice_signing = alice_wallet.sign_transaction(ALICE_PUBLIC, BOB_PUBLIC, 10.0)
    alice_node.blockchain.mine_block()
    alice_blockchain.add_transaction(BOB_PUBLIC, alice_signing, amount=10.0)
    alice_blockchain.add_transaction(BOB_PUBLIC, alice_signing, amount=10.0)
    assert len(alice_blockchain.view_open_transactions()) == 1
    os.remove(alice_blockchain.file_location)

def test_get_balance():
    alice_wallet = Wallet('./tests/alice_wallet.txt')
    alice_blockchain = BlockChain(ALICE_PUBLIC, './tests/blockchain.bin')
    alice_node = Node(alice_wallet, alice_blockchain)
    alice_signing = alice_wallet.sign_transaction(ALICE_PUBLIC, BOB_PUBLIC, 3.2)
    alice_node.blockchain.mine_block()
    alice_blockchain.add_transaction(BOB_PUBLIC, alice_signing, amount=3.2)
    alice_blockchain.add_transaction(BOB_PUBLIC, alice_signing, amount=3.2)
    alice_node.blockchain.mine_block()
    assert alice_blockchain.get_balance() == 13.6
    os.remove(alice_blockchain.file_location)

def test_add_transaction_to_open():
    alice_wallet = Wallet('./tests/alice_wallet.txt')
    alice_blockchain = BlockChain(ALICE_PUBLIC, './tests/blockchain.bin')
    alice_node = Node(alice_wallet, alice_blockchain)
    alice_signing = alice_wallet.sign_transaction(ALICE_PUBLIC, BOB_PUBLIC, 3.4)
    alice_node.blockchain.mine_block()
    alice_blockchain.add_transaction(BOB_PUBLIC, alice_signing, amount=3.4)
    alice_blockchain.add_transaction(BOB_PUBLIC, alice_signing, amount=3.4)
    assert repr(alice_blockchain.view_open_transactions()[0]) == 'Sender: {}, Recipient: {}, Amount: 3.4'.format(ALICE_PUBLIC, BOB_PUBLIC)
    assert repr(alice_blockchain.view_open_transactions()[1]) == 'Sender: {}, Recipient: {}, Amount: 3.4'.format(ALICE_PUBLIC, BOB_PUBLIC)
    assert isinstance(alice_blockchain.view_open_transactions()[0], Transaction)
    assert isinstance(alice_blockchain.view_open_transactions()[1], Transaction)
